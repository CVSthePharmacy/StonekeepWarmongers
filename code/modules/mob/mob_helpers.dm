...
/proc/firearm_recoil_mob(mob/M)
	var/base_pixel_x = M.pixel_x
	var/base_pixel_y = M.pixel_y
	...
	M.pixel_x = base_pixel_x
	M.pixel_y = base_pixel_y

	animate(
		M,
		pixel_x = base_pixel_x + pixel_x_diff,
		pixel_y = base_pixel_y + pixel_y_diff,
		time = 1,
...
/proc/firearm_recoil_camera(mob/M, duration, strength=1, direction)
	...
	// not a switch to allow for diagonals
	if(direction & NORTH)
	...
	// Normalize diagonal so magnitude stays consistent
	if(recoil_x && recoil_y)
		recoil_x *= 0.707
		recoil_y *= 0.707
...
/mob/proc/can_see_reagents()
	return stat == DEAD || has_unlimited_silicon_privilege || HAS_TRAIT(src, TRAIT_MEDIC) //Dead guys and silicons can always see reagents
