...
/obj/item/rogueweapon/spear/pike/ppu
	name = "pike"
	desc = "A long spear primarily used to halt charges in their tracks."
	icon_state = "pikep"
...
/obj/item/rogueweapon/spear/war
	force = 15
	force_wielded = 25
	possible_item_intents = list(SPEAR_THRUST, SPEAR_BASH) //bash is for nonlethal takedowns, only targets limbs
	gripped_intents = list(SPEAR_THRUST, SPEAR_CUT, SPEAR_BASH)
	name = "lance"
	desc = "A long spear primarily used by light cavalry."
	icon_state = "lancer"
	icon = 'icons/roguetown/weapons/64.dmi'
	pixel_y = -16
	pixel_x = -16
	...
	bigboy = TRUE
	gripsprite = TRUE
	wlength = WLENGTH_NORMAL //No free heart crits just for existing
	w_class = WEIGHT_CLASS_BULKY
	slot_flags = ITEM_SLOT_BACK
	max_blade_int = 100
	max_integrity = 300
...
/obj/item/rogueweapon/spear/war/ppu
	force = 15
	force_wielded = 25
	possible_item_intents = list(SPEAR_THRUST, SPEAR_BASH) //bash is for nonlethal takedowns, only targets limbs
	gripped_intents = list(SPEAR_THRUST, SPEAR_CUT, SPEAR_BASH)
